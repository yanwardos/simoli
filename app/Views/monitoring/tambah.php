					...
					<div class="card-body p-2 container">
						<div class="row">
							<div class="form-group col-lg-6 float-left">
								<label for="gedung" class="mr-sm-2 m-0">Sensor</label>
								<select name="gedung" id="gedung" class="custom-select mr-sm-2">
									<option selected value="-1">Pilih Sensor ...</option>
									<?php
									foreach ($sensors as $item)
									{
										?>
										<option value="<?php echo $item['id_sensor']?>"><?php echo $item['nama_sensor']?></option>
										<?php
									}?>
								</select>
							</div>
